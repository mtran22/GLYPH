...
expName = 'Stroop Hiraikana'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/mike/Desktop/Stroop Hiraikana.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    Variable = visual.TextStim(win=win, name='Variable',
        text='',
        font='Arial',
    ...
    polygon = visual.Rect(
        ...
        width=(0.30, 0.05)[0], height=(0.30, 0.05)[1],
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('Downloads/Surely v1.xlsx'), 
        seed=None, 
        isTrials=True, 
    )
    ...
    for thisTrial in trials:
        ...
        # create an object to store info about Routine Stroop_Task
        Stroop_Task = data.Routine(
            name='Stroop_Task',
            components=[Variable, polygon, key_resp_2],
        )
        Stroop_Task.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        ...
        key_resp_2.rt = []
        _key_resp_2_allKeys = []
        # store start times for Stroop_Task
        Stroop_Task.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        Stroop_Task.tStart = globalClock.getTime(format='float')
        Stroop_Task.status = STARTED
        thisExp.addData('Stroop_Task.started', Stroop_Task.tStart)
        Stroop_Task.maxDuration = None
        # keep track of which components have finished
        Stroop_TaskComponents = Stroop_Task.components
        for thisComponent in Stroop_Task.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        thisExp.currentRoutine = Stroop_Task
        Stroop_Task.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # if trial has changed, end Routine now
            if hasattr(thisTrial, 'status') and thisTrial.status == STOPPING:
                continueRoutine = False
            ...
            # if Variable is starting this frame...
            if Variable.status == NOT_STARTED and condition=='word':
                # keep track of start time/frame for later
                Variable.frameNStart = frameN  # exact frame index
                Variable.tStart = t  # local t and not account for scr refresh
                Variable.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(Variable, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'Variable.started')
                # update status
                Variable.status = STARTED
                Variable.setAutoDraw(True)
            
            # if Variable is active this frame...
            if Variable.status == STARTED:
                # update params
                Variable.setColor(color, colorSpace='rgb', log=False)
                Variable.setText(word, log=False)
                Variable.setFont(font, log=False)
            
            # if Variable is stopping this frame...
            if Variable.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > Variable.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
                    Variable.tStop = t  # not accounting for scr refresh
                    Variable.tStopRefresh = tThisFlipGlobal  # on global time
                    Variable.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'Variable.stopped')
                    # update status
                    Variable.status = FINISHED
                    Variable.setAutoDraw(False)
            
            # *polygon* updates
            
            # if polygon is starting this frame...
            if polygon.status == NOT_STARTED and condition=='shape':
                # keep track of start time/frame for later
            ...
            if key_resp_2.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_2.getKeys(keyList=['b','y','g','r'], ignoreKeys=["escape"], waitRelease=False)
            ...
            # has a Component requested the Routine to end?
            if not continueRoutine:
                Stroop_Task.forceEnded = routineForceEnded = True
            # has the Routine been forcibly ended?
            if Stroop_Task.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in Stroop_Task.components:
        ...
        for thisComponent in Stroop_Task.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for Stroop_Task
        Stroop_Task.tStop = globalClock.getTime(format='float')
        Stroop_Task.tStopRefresh = tThisFlipGlobal
        thisExp.addData('Stroop_Task.stopped', Stroop_Task.tStop)
        # check responses
